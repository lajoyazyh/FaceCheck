...
\label{tab:entities}
\footnotesize
\begin{tabular}{@{}>{\ttfamily}l p{4cm} p{3.8cm} @{}}
\toprule
实体 & 关键字段 & 约束/状态 \\
\midrule
User & username, password\_hash, role, status, lastLoginAt &
  表名 user\_account，ROLE: USER|ADMIN，STATUS: ACTIVE|LOCKED|DISABLED \\
...
FacePhoto & obsBucket, obsRegion, obsObjectKey, contentType,
  ...
  DETECT: PENDING|PASSED|FAILED，REGISTER: PENDING|ACTIVE|FAILED|DELETE\_PENDING|DELETE\_FAILED|DELETED \\

HuaweiFaceRef & userId, facePhotoId, faceSetName, frsFaceId,
                externalImageId, externalFields(JSONB), status &
  STATUS: ACTIVE|DELETE\_PENDING|DELETE\_FAILED|DELETED|ORPHANED \\

AttendanceSession & name, description, startTime, endTime,
                     lateAfterTime, status, qrToken &
  STATUS: DRAFT|PUBLISHED|CLOSED|CANCELED，qrToken 高熵唯一 \\

AttendanceCheckinAttempt & obsBucket, obsRegion, obsObjectKey,
                            contentType, sizeBytes, sha256,
                            status, resultCode, failureReason,
                            frsRequestId, matchedUserId, similarity,
                            idempotencyKey, reviewed &
  STATUS: PROCESSING|SUCCESS|FAILED|DUPLICATE\_CHECKIN，\\
  UK(session\_id, idempotency\_key) \\

AttendanceRecord & sessionId, userId, attemptId, checkinTime,
                   status, similarity, source &
  STATUS: VALID|MANUAL\_CONFIRMED，SRC: APP\_QR\_ANON|ADMIN\_RETRY，\\
...
ExternalServiceCallLog & serviceName, operation, requestId,
                          status, latencyMs, errorCode &
  SVC: HUAWEI\_FRS|HUAWEI\_OBS，STATUS: SUCCESS|FAILED|TIMEOUT|RATE\_LIMITED|RETRYING \\

SystemConfig & configKey, configValue, valueType, description &
  TYPE: STRING|NUMBER|BOOLEAN|JSON，白名单配置管理 \\
